...
int64_t GetBlockValue(CBlockIndex* pindex, int64_t nFees, bool noScale)
{
    ...
    if (onForkNow && !noScale) {
      ...
      if (!scalingFactor.convert_to<unsigned int>()) { // find the key block and recalculate
    ...
    if (RegTest()) {
      ...
      return nFees + baseSubsidy - ((BoostBigNum(baseSubsidy)*BoostBigNum(100000000))/scalingFactor).convert_to<unsigned int>()/2;
    }
...
BoostBigNum get_ssf (CBlockIndex * pindex) {
  ...
  if (hashes_peak > BoostBigNum(0) && hashes_cur != hashes_peak) {
    if (onFork2(pindex)) {
      ...
      scalingFactor = BoostBigNum(((100000000*hashes_peak)/(hashes_peak-hashes_cur)).convert_to<uint64_t>());
    }
  }
