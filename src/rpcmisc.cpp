...
Value mark(const Array& params, bool fHelp)
{
  if (fHelp || params.size() < 1 || params.size() > 3 || params.size() == 2)
    throw runtime_error(
			            "mark (data address amount)\n"
				    "\nArguments:\n"
				    "1. \"data\"     (string, optional) The data to imbed into the blockchain (max 64 characters).\n"
				    "2. \"address\"     (string, optional) A payment address, which can be used for tipping the author of the data\n"
  ...
  const char * txid = 0;
  int nOutput = -1;
  if (!data) throw runtime_error("Some data is needed\n");
  if (params.size()==3) {
    address = CBitmarkAddress(params[1].get_str());
    if (!address.IsValid())
      throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Bitmark address");
    nAmount = AmountFromValue(params[2]);
    EnsureWalletIsUnlocked();
    string strError = pwalletMain->SendMoneyToDestination(address.Get(), nAmount, wtx, data);
  }
  else {
    EnsureWalletIsUnlocked();
    LogPrintf("sendmoneytonodestination with nOutput=%d\n",nOutput);
    string strError = pwalletMain->SendMoneyToNoDestination(wtx,data);
  }
