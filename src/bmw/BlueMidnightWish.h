...
typedef enum { SUCCESS = 0, FAIL = 1, BAD_HASHLEN = 2, BAD_CONSECUTIVE_CALL_TO_UPDATE = 3 } BMWHashReturn;
...
BMWHashReturn BMWInit(hashState *state, int hashbitlen);
BMWHashReturn BMWUpdate(hashState *state, const BitSequence *data, DataLength databitlen);
BMWHashReturn BMWFinal(hashState *state, BitSequence *hashval);
BMWHashReturn BMWHash(int hashbitlen, const BitSequence *data, DataLength databitlen, BitSequence *hashval);
