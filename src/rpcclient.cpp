...
Array RPCConvertValues(const std::string &strMethod, const std::vector<std::string> &strParams)
{
    ...
    if (strMethod == "listtransactions"       && n > 3) ConvertTo<int64_t>(params[3]);
    if (strMethod == "listaccounts"           && n > 0) ConvertTo<int64_t>(params[0]);
    if (strMethod == "listaccounts"           && n > 1) ConvertTo<int64_t>(params[1]);
    if (strMethod == "walletpassphrase"       && n > 1) ConvertTo<int64_t>(params[1]);
    if (strMethod == "getblocktemplate"       && n > 0) ConvertTo<Object>(params[0]);
    if (strMethod == "listsinceblock"         && n > 1) ConvertTo<int64_t>(params[1]);
    if (strMethod == "listsinceblock"         && n > 2) ConvertTo<int64_t>(params[2]);
    ...
    if (strMethod == "coins" && n>0) ConvertTo<int64_t>(params[0]);
    if (strMethod == "coins" && n>1) ConvertTo<int64_t>(params[1]);
    if (strMethod == "mark") ConvertTo<Object>(params[0]);
    if (strMethod == "mark" && n>2) ConvertTo<double>(params[2]);
    return params;
}
...
std::string HelpMessageCli(bool mainProgram)
{
    ...
    strUsage += "  -rpcport=<port>        " + _("Connect to JSON-RPC on <port> (default: 9266 or testnet: 19266)") + "\n";
